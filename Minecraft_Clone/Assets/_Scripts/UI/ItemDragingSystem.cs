...
public class ItemDragingSystem : MonoBehaviour
{
    ...
    private void OnDisable()
    {
        MInput.UI_LeftClick.performed -= ProcessLeftClickInput;
        MInput.UI_RightClick.performed -= ProcessRightClickInput;
        if(!DraggingSlot.IsEmpty())
        {
            InventorySystem.Instance.AddItemToInventory(DraggingSlot);
        }
    }
    ...
    {
        RaycastUtilities.UIRaycast(MInput.PointerPosition, _rayCastResult);
        foreach (RaycastResult hit in _rayCastResult)
        {
